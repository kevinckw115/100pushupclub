...
try {
  ...
  await a.getByRole('tab', { name: 'Circles', exact: true }).click(); await expect(a.getByText('No circles yet. Create a small group or join with an invite.')).toBeVisible({ timeout: 20000 });
  ...
  await button(a, 'Accept participation terms').click(); await button(a, 'Save account settings').click(); await expect(a.getByText(/Last confirmed sharing: off\. Only future eligible/)).toBeVisible({ timeout: 20000 });
  ...
  await expect(a.getByText(/Members can see your new check-ins/)).toBeVisible(); await button(a, 'Accept circle sharing').click();
  await a.context().setOffline(true); await button(a, 'Create my circle').click(); await expect(a.getByText(/Your circle request is saved on this device/)).toBeVisible();
  ...
  await a.context().grantPermissions(['clipboard-read', 'clipboard-write']); await button(a, 'Copy invite code').click(); await expect(a.getByText('Invite code copied.', { exact: true })).toBeVisible();
  ...
  await expect(a.getByText(/2 of 3 members checked in/)).toBeVisible(); await expect(a.getByText('No check-in yet today', { exact: true })).toBeVisible(); await expect(a.getByText(/America\/Los_Angeles · Fixed circle timezone/)).toBeVisible();
  ...
  await a.context().setOffline(true); await expect(a.getByText(/Circle activity is hidden until current membership/)).toBeVisible({ timeout: 20000 }); await expect(a.getByLabel('185 circle pushups today', { exact: true })).toHaveCount(0);
  await a.context().setOffline(false); await expect(a.getByLabel('185 circle pushups today', { exact: true })).toBeVisible({ timeout: 25000 });
  await button(b, 'Manage circle').click(); await button(b, 'Leave circle').click(); await button(b, 'Confirm leave').click(); await expect(b.getByText(/This circle or member is no longer available/)).toBeVisible({ timeout: 20000 });
  await button(b, 'Back to circles').click(); await expect(b.getByText('No circles yet. Create a small group or join with an invite.')).toBeVisible({ timeout: 20000 });
  ...
  await button(a, 'Leave circle').click(); await button(a, 'Confirm leave').click(); await expect(a.getByText(/This circle or member is no longer available/)).toBeVisible({ timeout: 20000 });
  await button(b, 'Manage circle').click(); await expect(button(b, 'Delete circle')).toBeVisible({ timeout: 20000 }); await button(b, 'Delete circle').click(); await button(b, 'Confirm delete').click(); await expect(b.getByText(/This circle or member is no longer available/)).toBeVisible({ timeout: 20000 });
  await button(b, 'Back to circles').click(); await expect(b.getByText('No circles yet. Create a small group or join with an invite.')).toBeVisible({ timeout: 20000 });
