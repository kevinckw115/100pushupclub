...
- `npm run export:web`: browser bundle audited for privileged credentials and review/test modules.
...
Copy .env.example to .env for local settings. Preview requires its own explicit HTTPS backend and public API key; privileged keys are rejected. Optional support email remains unset until supplied by the owner. See ../../docs/12-operations-runbook.md for hosted setup and ../../docs/13-beta-review.md for the native acceptance matrix.
