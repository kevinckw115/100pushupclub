...
### 2026-09-21 / ADR-012 / Staging website and scheduled cleanup hosting

The owner confirmed Vercel access, support forwarding to their Gmail, and a preference for no additional paid worker host. A standalone static website serves support, policies and the existing verified deletion RPC flow without publishing the mobile web tracker. Supabase Cron invokes an Edge wrapper around the existing deletion worker every minute, using a separate256-bit secret in Vault and Edge secrets. Client sessions/public keys do not authorize this endpoint; a session-level advisory lock excludes overlapping runs. An optional external heartbeat detects failed or missing runs without receiving account data. Render deployment files were removed from the final branch. Public deletion/data/sync contracts are unchanged; no new app RPC, migration or production host is activated by this decision. Free-tier quotas, actual hosted connectivity/alert delivery, backup/retention and native gates remain subject to verification.
